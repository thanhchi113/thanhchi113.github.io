...
(async () => {
    ...
    try {
        for (const mobile of [false, true]) {
            ...
            assert.equal((await styles(page, '#achievementGallery')).backdrop, 'none');
            assert.equal((await styles(page, '#achievementGallery')).background, 'rgba(0, 0, 0, 0)', 'Whole-card viewer preserves the galaxy without any extra tint');
            assert(await page.locator('#achievementGalleryContent .evidence-content').isVisible(), 'Full evidence information accompanies its image');
            ...
            assert(await page.locator('#achievementGallery').isHidden());
            ...
            assert.equal(await page.evaluate(() => document.activeElement.id), 'achievementGalleryClose');
            assert(await page.locator('#achievementGalleryContent .exam-student-info').isVisible(), 'Score information accompanies the enlarged evidence');
            ...
            assert(await page.locator('#achievementGallery').isHidden());
            assert(await page.evaluate(() => document.activeElement.matches('.exam-student-card')), 'Escape returns focus to the score card');
